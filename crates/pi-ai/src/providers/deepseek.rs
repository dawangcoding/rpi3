...
//! DeepSeek 提供 DeepSeek V3 和 R1 推理模型
...
impl ApiProvider for DeepSeekProvider {
    ...
    async fn stream(
        ...
        // 委托给 OpenAI Provider（DeepSeek API 完全兼容 OpenAI 格式）
...
mod tests {
    ...
    fn test_deepseek_api_type() {
        ...
        assert_eq!(provider.api(), Api::DeepSeek);
    }

    #[tokio::test]
    async fn test_deepseek_stream_text_response() {
        let mut server = Server::new_async().await;
        let provider = DeepSeekProvider::new();

        // DeepSeek SSE 格式（与 OpenAI 兼容）
        let sse_body = r#"data: {"id":"chatcmpl-123","object":"chat.completion.chunk","created":1677652288,"model":"deepseek-chat","choices":[{"index":0,"delta":{"role":"assistant"},"finish_reason":null}]}
...
data: {"id":"chatcmpl-123","object":"chat.completion.chunk","created":1677652288,"model":"deepseek-chat","choices":[{"index":0,"delta":{"content":" from DeepSeek!"},"finish_reason":null}]}
...
"#;
        ...
        let mut model = sample_model(Api::DeepSeek, Provider::DeepSeek);
        ...
        assert!(text_deltas.iter().any(|d| d.contains("DeepSeek")), "Should contain 'DeepSeek' in response");

        mock.assert_async().await;
    }

    #[tokio::test]
    async fn test_deepseek_stream_tool_call() {
        let mut server = Server::new_async().await;
        let provider = DeepSeekProvider::new();

        let sse_body = r#"data: {"id":"chatcmpl-456","object":"chat.completion.chunk","created":1677652288,"model":"deepseek-chat","choices":[{"index":0,"delta":{"role":"assistant","tool_calls":[{"index":0,"id":"call_abc123","type":"function","function":{"name":"get_weather","arguments":"{\"location\":\"Paris\"}"}}]},"finish_reason":null}]}

data: {"id":"chatcmpl-456","object":"chat.completion.chunk","created":1677652288,"model":"deepseek-chat","choices":[{"index":0,"delta":{},"finish_reason":"tool_calls"}]}
